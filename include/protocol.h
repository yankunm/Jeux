 ...
 *   (1) LOGIN:    Log a username into the server
 *             Payload: username
 *   (2) USERS:    Request a list of currently logged-in users
 *   (3) INVITE:   Invite another user to a game
 *             Header: role to which the target is invited.
 *             Payload: username of target
 *   (4) REVOKE:   Revoke an invitation previously made
 *             Header: invitation ID assigned by the source
 *   (5) ACCEPT:   Accept an invitation made by another player
 *             Header: invitation ID assigned by the target
 *   (6) DECLINE:  Decline an invitation made by another player
 *             Header: invitation ID assigned by the target
 *   (7) MOVE:     Make a move in an ongoing game
 *             Header: invitation ID assigned by player making the move
 *             Payload: string that describes the move
 *   (8) RESIGN:   Resign an ongoing game
 *             Header: invitation ID assigned by player who is resigning
 *
 * Server-to-client responses (synchronous):
 *   (9) ACK:      Sent in response to a successful request
 ...
 *   (10) NACK:     Sent in response to a failed request
 *
 * Server-to-client notifications (asynchronous):
 * Specific to client:
 *   (11) INVITED   Sent when an invitation has been made
 *             Header: invitation ID
 *             Payload: user name of source
 *   (12) REVOKED   Sent when an invitation has been revoked by source
 *             Header: invitation ID assigned by target
 *   (13) ACCEPTED  Sent when an invitation has been accepted by target
 *             Header: invitation ID assigned by source
 *             Payload: string showing initial game state
 *   (14) DECLINED  Sent when an invitation has been declined by target
 *             Header: invitation ID assigned by source
 *   (15) MOVED     Sent when the opponent has made a move
 *             Header: invitation ID
 *             Payload: string showing game state after the move
 *   (16) RESIGNED  Sent when the opponent has resigned
 *             Header: invitation ID assigned by recipient
 *   (17) ENDED     Sent when a game has ended
 *             Header: invitation ID assigned by recipient
